...
export default {
  TextNode: (node, _, style, props) => (
    <Text key={node.selectors[0]} style={style} {...props.passProps}>
      {node.data}
    </Text>
  ),
  p: (node, renderChildren, style, props) => (
    <View key={node.selectors[0]} style={style} {...props.passProps}>
  ...
  code: (node, renderChildren, style, props) => (
    <View key={node.selectors[0]} style={style} {...props.passProps.passProps}>
      {renderChildren(node.children)}
    </View>
  ),
  ...
  img: (node, __, style, props) => (
    <Image key={node.selectors[0]} style={style} source={{ uri: props.attributes?.src }} {...props.passProps} />
  ),
  ...
  button: (node, renderChildren, style, props) => (
    <TouchableHighlight key={node.selectors[0]} style={style} {...props.passProps}>
      {renderChildren(node.children)}
    </TouchableHighlight>
  ),
